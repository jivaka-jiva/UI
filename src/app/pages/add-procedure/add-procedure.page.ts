...
export class AddProcedurePage implements OnInit {
  ...
  ionViewWillEnter() {
    this.initializeForms();
    this.addProcedureForm.get('hospital').valueChanges
      .subscribe(value => {
  ...
  initializeForms() {
    this.addProcedureForm = new FormGroup({
      ...
      procedures: new FormControl({value: [], disabled: true}, [
        Validators.required
      ])
    });
  ...
  addProcedure(procedureDetails: FormGroup[]) {
    ...
    procedureDetails.forEach(procedureGroup => {
      const procedure = new FormGroup({
        hospital: new FormControl(this.addProcedureForm.get('hospital').value, [
          Validators.required
        ]),
        procedure: new FormControl(procedureGroup.value.procedure, [
          Validators.required
        ]),
        count: new FormControl(procedureGroup.value.count, [
          Validators.min(1)
        ]),
        amount: new FormControl(procedureGroup.value.amount, [
          Validators.required,
          Validators.min(0)
        ]),
  ...
  submit() {
    if (this.procedureForm.valid) {
      ...
      setTimeout(() => {
        this.router.navigateByUrl('/dashboard').then(() => this.authService.dismissLoader());
      }, 1000);
    }
  }
  ...
  async saveData() {
    ...
    procedureList.forEach(procedure => {
      ...
      hospitalData[procedure.value.hospital] = parseInt(hospitalData[procedure.value.hospital], 10) +
        (parseInt(procedure.value.amount, 10) * parseInt(procedure.value.count, 10));
    });
